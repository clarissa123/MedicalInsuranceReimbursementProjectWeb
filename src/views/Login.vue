    ...
    <div class="logForm">
      ...
      <div class="right">
        <el-form ref="loginForm" :model="loginForm">
          <el-form-item>
            <div style="fontSize:24px;color:rgba(51,51,51,1);fontWeight:bold;text-align:center">账号密码登录</div>
          </el-form-item>
          ...
          <el-form-item prop="pwd" :rules="[{ required: true, message: '请输入密码'}]">
            <el-input v-model="loginForm.pwd" type="password" @keyup.enter.native="onSubmit" prefix-icon="el-icon-lock"
              placeholder="请输入密码"></el-input>
          </el-form-item>
          <el-form-item>
            <el-checkbox v-model="checked" @change="clickRemember">记住密码</el-checkbox>
          </el-form-item>
          <el-button type="primary" @click="onSubmit" class="btn">点击登录</el-button>
        </el-form>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      clickRemember() {
        if (this.checked) {
          //保存到 cookie
          setCookie(this.loginForm.account, this.loginForm.pwd);
        } else {
          //清除cookie
          delCookie(this.loginForm.account);
        }
      },
      ...
      onSubmit() {
        this.$refs.loginForm.validate((valid) => {
          if (valid) {
            ...
            let param = this.base64.encode(JSON.stringify(this.loginForm));
            //加入干扰字符
            let Num = MathRand();
            param = param.slice(0, 5) + Num + param.slice(5);
            Api.user.login(param).then((res) => {
              if (res.status === 200) {
                //清空localStorage和sessionStorage
                clearSessionStorage();
                clearStorage();
                //把token放入sessionStorage里保存
                window.sessionStorage.setItem("token", res.data.token);
                // 把用户名放入sessionStorage里保存
                window.sessionStorage.setItem("userInfo", JSON.stringify(res.data.user));
                window.sessionStorage.setItem("userName", res.data.user.userAccount);
                //将接口权限列表保存到localStorage
                this.$store.dispatch("setAuthorityToLocalStorage",res.data.port);
                //将菜单权限列表保存到localStorage
                if (res.data.menu.length == 0) {
                  this.$message({
                    showClose: true,
                    message: "您没有菜单权限，请联系管理员！",
                    type: "warning",
                  });
                  return false;
                } else {
                  //给菜单设置格式
                  setMenuTree(res.data.menu);
                  //将菜单权限列表保存到localStorage
                  this.$store.dispatch("setLocalStorage", res.data.menu);
                  this.$message({
                    duration: 500,
                    type: "success",
                    message: "登录成功！"
                  ...
                  this.$router.push({
                    name: this.$router.options.routes[2].children[0].children[0].name
                  });
                }
              } else {
